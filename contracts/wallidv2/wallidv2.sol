...
contract WalletIdHydrid 
{
   ...
   function addUserBlock(uint256  uIdx, bytes32 idt, uint8 idx , bytes calldata dblock , bytes32 sid ) external returns (uint256 sucess, bytes32 code)
   {
       ...
       iData.idt_type[idt].sid = sid;
       iData.idt_type[idt].dBlocks[idx] = dblock;
   ...
   function getUserBlock(uint256 uIdx, uint nonce , bytes32 idt, uint8 idx) external view returns (uint256 sucess, bytes memory uBlock, uint256 o_uIdx, bytes32 o_sid,  bytes32 o_idt)
   {
       ...
       if(idx > 9 && idx < 0)
       {
           return (0, bytes(''), uIdx, uBlockData.sid, idt);
       }
    
        return (1, uBlockData.dBlocks[idx],   uIdx, uBlockData.sid, idt);
   }
